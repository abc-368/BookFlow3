...
namespace NinjaTrader.NinjaScript.Indicators
{
    public class BookFlowIndi : Indicator
    {
        ...
        [Display(Name = "Ticker ID", Description = "Assigned ticker ID from global AddOn", Order = 6, GroupName = "Status")]
        ...
        protected override void OnMarketData(MarketDataEventArgs e)
        {
            try
            {
                if (BookFlowAddOn.Instance == null || _tickerId == 0) return;
                ...
                LogLevel1($"T{message.TickerId:D2} {e.MarketDataType,-4} P:{FormatPrice(e.Price),-10} V:{FormatVolume(e.Volume),-8} B:{FormatPrice(e.Bid),-10} A:{FormatPrice(e.Ask),-10} TS:{FormatTimestamp(e.Time.Ticks)} S#{FormatSequence(seq)}");
                BookFlowAddOn.Instance.WriteToGlobalChannel(message.TickerId, message);
            }
        ...
        protected override void OnMarketDepth(MarketDepthEventArgs e)
        {
            try
            {
                ...
                var message = new UnifiedMarketDataMessage
                {
                    ...
                    NtReceiveTime = ntReceiveTime,
                    Sequence = seq,
                    Reserved1 = 0 // do not mark snapshots heuristically; send raw
                };
                LogLevel2($"T{message.TickerId:D2} {e.Operation,-3} {(e.MarketDataType == MarketDataType.Ask ? "ASK" : "BID"),-3} @{e.Position:D2} P:{FormatPrice(e.Price),-10} V:{FormatVolume(e.Volume),-8} TS:{FormatTimestamp(e.Time.Ticks)} S#{FormatSequence(seq)}");
                BookFlowAddOn.Instance.WriteToGlobalChannel(message.TickerId, message);
            }
